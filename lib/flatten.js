 ...
 * @param {Object} options Options available: info and verbose 
  ...
  if (fs.existsSync(resolvedDir)) {
    ...
    walkModules(resolvedDir, function(err, modules) {
      if (err) return callback(err);
        ...
        var seenData = [];
        var duplicateData = [];
        var firstLevelModuleCount = 0;
        var deeperLevelModuleCount = 0;
        ...
        for (var i = 0; i < modules.length; i++) {
          ...
          if (occurence.node_modules === 1) firstLevelModuleCount++;
          else deeperLevelModuleCount++;
        ...
        if (options.verbose) {
          ...
          console.log('         ', colors.cyan(firstLevelModuleCount), 'packages at first node_modules level');
          console.log('         ', colors.cyan(deeperLevelModuleCount), 'packages at deeper node_modules level');
        }
